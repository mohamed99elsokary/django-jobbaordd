...
def _save(im, fp, filename):
    ...
    logger.debug(f"Tiffinfo Keys: {list(info)}")
    ...
    if libtiff:
        ...
        logger.debug(f"Items: {sorted(ifd.items())}")
        _fp = 0
        ...
        legacy_ifd = im.tag.to_v2() if hasattr(im, "tag") else {}
        for tag, value in itertools.chain(
            ...
            if tag not in TiffTags.LIBTIFF_CORE:
                ...
                if tag in ifd.tagtype:
                    ...
                    if type := TiffTags.lookup(tag).type:
                        types[tag] = type
        ...
        logger.debug(f"Converted items: {sorted(atts.items())}")
    ...
    try:
        with AppendingTiffWriter(fp) as tf:
            for ims in [im] + append_images:
                ...
                nfr = ims.n_frames if hasattr(ims, "n_frames") else 1
