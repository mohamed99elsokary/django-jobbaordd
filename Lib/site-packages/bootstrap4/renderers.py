...
class BaseRenderer(object):
    ...
    def get_size_class(self, prefix="form-control"):
        if self.size == "small":
            return f"{prefix}-sm"
        return f"{prefix}-lg" if self.size == "large" else ""
...
class FormsetRenderer(BaseRenderer):
    ...
    def render_errors(self):
        if formset_errors := self.get_formset_errors():
...
class FormRenderer(BaseRenderer):
    ...
    def __init__(self, form, *args, **kwargs):
        ...
        self.form = form
        super().__init__(*args, **kwargs)
        self.error_css_class = kwargs.get("error_css_class")
        self.required_css_class = kwargs.get("required_css_class")
        self.bound_css_class = kwargs.get("bound_css_class")
...
class FieldRenderer(BaseRenderer):
    ...
    def list_to_class(self, html, klass):
        ...
        if enclosing_div := soup.find("div", {"class": classes}):
    ...
    def fix_date_select_input(self, html):
        ...
        div2 = "</div>"
        html = html.replace("<select", f"{div1}<select")
        html = html.replace("</select>", f"</select>{div2}")
    ...
    def append_help(self, html):
        if field_help := self.field_help or None:
    ...
    def append_errors(self, html):
        if field_errors := self.field_errors:
    ...
    def wrap_field(self, html):
        if field_class := self.get_field_class():
